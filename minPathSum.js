var minPathSum = function (grid) {
    ...
    for (let i = 0; i < grid.length; i++ ) {
        ...
        for (let j = 0; j < grid[i].length; j++ ) {
            let num = grid[i][j];
            let left = grid[i][j - 1] !== undefined ? grid[i][j - 1] + num : undefined;
            let top = grid[i - 1] !== undefined ? grid[i - 1][j] + num : undefined;

            if ( left || top ) {
                if ( left && top ) {
                    grid[i][j] = left < top ? left : top;
                } else {
                    grid[i][j] = left ? left : top;
                }
            }
        }
    }
...
let input = [[0, 2, 2, 6, 4, 1, 6, 2, 9, 9, 5, 8, 4, 4], 
